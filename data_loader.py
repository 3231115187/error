...
class data_loader:
        ...
        with open(os.path.join(self.path, 'link.dat'), 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if h_id in self.old_to_new_id_mapping.keys() and t_id in self.old_to_new_id_mapping.keys():
                    ...
                    if r_id not in links['meta']:  # 空
                        h_type = self.get_node_type(h_id)  # 获取节点类型,返回0,1,2：病人、药物、程序
                        t_type = self.get_node_type(t_id)
                        links['meta'][r_id] = (h_type, t_type)  # r_id：1,2,3,4(pd、po、dp、op)
                    links['data'][r_id].append((h_id, t_id, link_weight))
                    links['count'][r_id] += 1
                    links['total'] += 1
                    if r_id not in r_ids:
                        r_ids.append(r_id)
        r_ids = sorted(r_ids)  # [0, 1, 2, 3]
    ...
    def load_nodes(self):
        """
        return nodes dict
            1.total: total number of nodes
            2.count: a dict of int, number of nodes for each type
            3.attr: a dict of np.array (or None), attribute matrices for each type of nodes
            4.shift: node_id shift for each type. You can get the id range of a type by
        ...
        with open(os.path.join(self.path, 'node.dat'), 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if len(th) == 4:
                    ...
                    node_attr = list(map(float, node_attr.split(',')))  # 取出attr中的, 我可以得到一半的数据，即(*2901)
                    ...
                    node_ids.append(node_id)
                    nodes['count'][node_type] += 1
                    nodes['total'] += 1  # 1.total
                else:
                    raise Exception("Too few information to parse!")

        # type_id mapping == 2.count    ???感觉没有变化
        temp_count = Counter()
        for k in nodes['count']:  # [0: 9860, 1: 1460, 2: 1439]
            temp_count[k] = nodes['count'][k]
        nodes['count'] = temp_count

        # node_id mapping   ???感觉没变化
        ...
        for old_id in node_ids:
            if old_id in nodes['attr'].keys():  # [0,1,3...,12759]
                temp_attr[self.old_to_new_id_mapping[old_id]] = nodes['attr'][old_id]
        nodes['attr'] = temp_attr

        shift = 0
        attr = {}
        for i in range(len(nodes['count'])):  # 0,1,2
            nodes['shift'][i] = shift  # 4.shift
            if shift in nodes['attr']:  # 12759行    ->  (12759*2901)
                mat = []
                for j in range(shift, shift + nodes['count'][i]):  # 0~9860,9860~12759,
                    mat.append(nodes['attr'][j])
                attr[i] = np.array(mat)  # 9860, 1460, 1439
            else:
                attr[i] = None
            shift += nodes['count'][i]
        nodes['attr'] = attr  # 3.attr
        return nodes  # √
